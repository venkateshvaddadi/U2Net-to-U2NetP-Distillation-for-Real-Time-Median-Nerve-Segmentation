...
from models.u2_net_model import U2NET, U2NETP
from CTS_dataset_updated_for_BASNet import mydataloader
...
class DistillationLoss(nn.Module):
    ...
    def forward(self, s_outputs, t_outputs, target):
        # -------------------------
        # Ground Truth Loss (original)
        # -------------------------
        loss_gt = muti_bce_loss_fusion_6(*s_outputs, target)
        ...
        for s, t in zip(s_outputs, t_outputs):
            loss_kd += bce_ssim_loss(s, t)

        # -------------------------
        # Final Loss
        # -------------------------
        loss = self.alpha * loss_gt + (1 - self.alpha) * loss_kd
...
#%%
#%%
from models.u2_net_model import U2NET
import torch
